...
class GroqService {
    ...
    private var suggestionSystemPrompt: String {
        if appLanguage == "en" {
            return """
            You are a personal nutrition coach. \
            You will suggest meals based on the user's remaining daily macros and preferences.

            Afternoon/Evening notification (16:00):
            - Suggest a full meal for dinner
            - Athlete-friendly food to complete missing macros
            - Use practical, accessible examples
            - You can say "pick up X from the store" if needed

            Night closing notification (21:30):
            - Light, easy to digest, suitable before sleep
            - Protein-focused small snack
            - Examples: kefir, yogurt, eggs, protein shake
            - Suggest things that won't upset the stomach

            Respond ONLY in JSON format, write nothing else. Respond in English.
            """
        } else {
            return """
            Sen kişisel bir Türk beslenme koçusun. \
            Kullanıcının günlük kalan makrolarına ve tercihlerine göre \
            yemek önerisi yapacaksın.

            Öğle/Akşam bildirimi (16:00) için:
            - Akşam yemeği için tam öğün öner
            - Eksik makroları tamamlayacak sporcu dostu yemek
            - Türk mutfağından örnekler kullan
            - Gerekirse "marketten şunu al" diyebilirsin

            Gece kapanış bildirimi (21:30) için:
            - Hafif, sindirimi kolay, uyku öncesi uygun
            - Protein ağırlıklı küçük atıştırmalık
            - Örnekler: kefir, yoğurt, yumurta, protein süt
            - Mideyi yormayacak şeyler öner

            SADECE JSON formatında yanıt ver, başka hiçbir şey yazma. SADECE Türkçe yanıt ver.
            """
        }
    }
    ...
    func generateMealSuggestion(
        ...
        if lang == "en" {
            typeText = notificationType == .afternoon ? "Dinner suggestion" : "Night snack suggestion"
            mealsText = todayMeals.isEmpty ? "Nothing eaten yet" : todayMeals.joined(separator: ", ")
        } else {
            typeText = notificationType == .afternoon ? "Akşam yemeği önerisi" : "Gece atıştırmalığı önerisi"
            mealsText = todayMeals.isEmpty ? "Henüz bir şey yenmedi" : todayMeals.joined(separator: ", ")
        }
        let activityNames = todayActivities
            .compactMap { GoalEngine.activityDisplayNames[$0] }
            .joined(separator: ", ")
        ...
        if lang == "en" {
            userPrompt = """
                ...
                Today's meals: \(mealsText)
                Preferred proteins: \(preferredProteins.joined(separator: ", "))
                Today's activity: \(activityNames)
                Recovery status: \(hrvStatus.rawValue)
            ...
            userPrompt = """
                ...
                Kalan yağ: \(remainingFat)g
                Bugün yenenler: \(mealsText)
